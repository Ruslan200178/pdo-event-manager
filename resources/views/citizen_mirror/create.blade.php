...
@section('title', 'Add Citizen Mirror')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 tracking-tight">Add Citizen Mirror Entry</h1>
            <p class="text-xs text-gray-500">Record public survey findings, local feedback comments, and event outcomes.</p>
        </div>
        <div>
    ...
    <div class="bg-white rounded-2xl border border-gray-150 shadow-sm overflow-hidden">
        <form action="{{ route('citizen-mirror.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="p-6 md:p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" required placeholder="Waste Management feedback seminar" value="{{ old('title') }}" class="mt-1.5 block w-full px-3 py-2 text-sm border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-govblue-500 focus:border-govblue-500">
                    </div>

                    <!-- DS Division -->
                    <div>
                        <label for="division" class="block text-xs font-semibold text-gray-700 tracking-wide">DS Division</label>
                        <input type="text" name="division" id="division" required placeholder="Maharagama" value="{{ old('division') }}" class="mt-1.5 block w-full px-3 py-2 text-sm border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-govblue-500 focus:border-govblue-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- File Upload -->
                    <div>
                        <label for="document" class="block text-xs font-semibold text-gray-700 tracking-wide">Document / Photo Attachment <span class="text-gray-400 font-normal">(Images/PDFs)</span></label>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="5" placeholder="Summary of public satisfaction and officer suggestions..." class="mt-1.5 block w-full px-3 py-2 text-sm border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-govblue-500 focus:border-govblue-500">{{ old('description') }}</textarea>
                </div>
            </div>
