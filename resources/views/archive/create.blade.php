...
@section('title', 'Archive Document')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 tracking-tight">Archive Document</h1>
            <p class="text-xs text-gray-500">Log past projects, evaluations, or administrative document attachments.</p>
        </div>
        <div>
            ...
            <div class="p-6 md:p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="module_name" id="module_name" required class="mt-1.5 block w-full px-3 py-2 text-sm border border-gray-300 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-govblue-500 focus:border-govblue-500">
                            ...
                            @foreach($modules as $m)
                                <option value="{{ $m }}" {{ old('module_name') == $m ? 'selected' : '' }}>{{ $m }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" required value="{{ old('title') }}" class="mt-1.5 block w-full px-3 py-2 text-sm border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-govblue-500 focus:border-govblue-500">
                    </div>

                    <!-- File Upload -->
                    <div>
                        <label for="file" class="block text-xs font-semibold text-gray-700 tracking-wide">Attachment File <span class="text-gray-400 font-normal">(PDF, Word, Excel up to 10MB)</span></label>
                        <input type="file" name="file" id="file" required accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip,image/*" class="mt-1.5 block w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-govblue-50 file:text-govblue-900 hover:file:bg-govblue-100 border border-gray-300 rounded-xl">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="5" class="mt-1.5 block w-full px-3 py-2 text-sm border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-govblue-500 focus:border-govblue-500">{{ old('description') }}</textarea>
                </div>
            </div>
